...
**Hub:**
- `announcements` · `messageboard` · `library` · `cambridge-calendar` · `cambridge-standards` · `orientation` · `surveys`. The 4 Communications-family pages (announcements + messageboard + documents + cambridge-calendar) adopted the canonical `.page-footer` 2026-05-23/24 in a single design-system pass.
- **`references` (2026-05-09, parity refactor 2026-05-23/25)** — narrowed TH variant of the references hub (3 tabs · 15 docs: Frameworks · Cambridge · Permendiknas; ES + AICF mirror data added 2026-05-25 with cat-class colour wiring). Now consumes the shared `shared-design/references-shell` (CSS + ES module) for UI parity with CH and AH. **Local references-data mirror landed 2026-05-25** — previously fetched cross-origin from `centralhub.eduversal.org/references-data/*`; now `build.js` copies references-data into `dist/` killing the cross-origin SPOF. Hero + body + footer all hit a true 1200 px content rail (see root CLAUDE.md Common Mistakes #53 + #54 for the alignment / `body:has()` flex trap). Modal viewer auto-wires CTS / SKL / PIGP / ES / AICF chips via `cambridge-crossref.js`.
- **`handbook` (cross-hub redesign 2026-05-25/26)** — Browser shows the shared bookshelf strip + spine modal + canonical `.page-footer` + info strip. Reader is the same shared `handbook-reader.css` module as CH and AH consume. See root CLAUDE.md "Handbook Ecosystem" for layout discipline.
- **`competency-framework` + `my-portfolio` (canonical footer + info strip, 2026-05-23/25)** — adopted the canonical Knowledge-family footer + page info strip alongside the v3 TH refresh (7 domains × 28 comps + 4 chip families). See `memory/project_th_competency_framework_v3_refresh_2026_05_22.md` for the framework work.
...
- **Candidate emails go through `partials/mailer.js` → Resend mail-service.** Don't write to `mail/{auto}` (legacy Trigger Email path — retired 2026-05-06 because `noreply@firebaseapp.com` landed in spam). Pages that send mail must include `<script src="partials/mailer.js"></script>` BEFORE their main script. Sends are non-fatal — failure logs to console + caller toast, never blocks the underlying Firestore write. See [Careers + Interview Module](#careers--interview-module) Email flow for templateName options.
- **TH `auth-guard.js` dispatches `authReady` on `document`, NOT `window`.** Listening on the wrong target makes the listener never fire — page renders the empty shell because the init code that wires UI / starts onSnapshot / fetches data never runs, and the failure is **silent** (no errors, no toast, no network rejection). When porting code from Students Hub (which dispatches on `window`), always swap the listener target to `document.addEventListener('authReady', …)`. CH and AH do the same as TH; SH is the odd one out. Easy to miss because both forms are valid JS — only one matches the dispatcher. See CH CLAUDE.md Common Mistake #13 for the past-incident detail.
